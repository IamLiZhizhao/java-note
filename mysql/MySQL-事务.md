...
ps：不想引入gap lock，可以考虑改用RC隔离级别 + binlog_format=row

#### 锁冲突关系
行锁：跟行锁有冲突关系的是“另外一个行锁”。
...
加锁分两次，第一次，即正序的锁，第二次为倒序的锁，即多出的(5,10],
在RR隔离级别，innodb在加锁的过程中会默认向后锁一个记录，加上Next-Key Lock,第一次加锁的时候10已经在范围，
由于倒序，向后，即向5再加Next-key Lock,即多出的(5,10]范围